...
export default function SectionPage(props: SectionPageProps) {
    ...
    return(
        <>
        {section ? 
            <SectionHeader actions={sectionDetails?.result?.body?.actions.concat(subjectsList?.result?.body?.actions || [])} priority={checkPriority()} section={section} handleEditSection={handleEditSection} handleAddSubject={handleAddSubject}/> 
        : null }
        {subjects && subjects.length ?  
            <Container>
                <SubjectsList entities={subjectsList?.result?.body?.entities} subjects={subjects} handleDeleteSubject={handleDeleteSubject} handleEditSubject={handleEditSubject}/>
            </Container>
            :
